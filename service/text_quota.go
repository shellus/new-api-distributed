...
func collectToolSurchargeItem(items []ToolSurchargeItem, relayInfo *relaycommon.RelayInfo, name string, count int, modelName string) []ToolSurchargeItem {
	if count <= 0 {
		return items
	}
	price := resolvedBillingToolPrice(relayInfo, name, modelName)
...
func calculateTextToolCallSurcharge(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, summary *textQuotaSummary) decimal.Decimal {
	...
	dQuotaPerUnit := decimal.NewFromFloat(edgeQuotaPerUnit(relayInfo))
	...
	if relayInfo.ResponsesUsageInfo != nil {
		for name, tool := range relayInfo.ResponsesUsageInfo.BuiltInTools {
			...
			items = collectToolSurchargeItem(items, relayInfo, name, tool.CallCount, summary.ModelName)
		}
	}
	if relayInfo.RelayMode != relayconstant.RelayModeResponses &&
		strings.HasSuffix(summary.ModelName, "search-preview") {
		items = collectToolSurchargeItem(items, relayInfo, dto.BuildInToolWebSearchPreview, 1, summary.ModelName)
	}
	...
	if ctx.GetBool("gemini_google_search_call") {
		items = collectToolSurchargeItem(items, relayInfo, dto.BuildInToolGoogleSearch, 1, summary.ModelName)
	}
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.AudioInputPrice > 0 && summary.AudioTokens > 0 {
		q := decimal.NewFromFloat(summary.AudioInputPrice).
	...
	if common.IsEdgeMode() {
		...
		if len(summary.ToolSurchargeItems) > 0 {
			facts.ToolCalls = make(map[string]int, len(summary.ToolSurchargeItems))
			for _, item := range summary.ToolSurchargeItems {
				facts.ToolCalls[item.Name] += item.Count
			}
		}
