...
func TestRecoverEdgeTaskBillingReconcilesSettledAndZeroQuotaFailedTasks(t *testing.T) {
	...
	reloadedSettled, exists, err := model.GetByTaskId(settledTask.UserId, settledTask.TaskID)
	require.NoError(t, err)
	...
	require.NoError(t, RecoverEdgeTaskBilling(context.Background()))
	reloadedFailed, exists, err := model.GetByTaskId(failedTask.UserId, failedTask.TaskID)
	require.NoError(t, err)
	require.True(t, exists)
...
func TestRecoverEdgeTaskBillingQuarantinesOneOwnerMismatchAndContinues(t *testing.T) {
	...
	reloadedAnomalous, exists, err := model.GetByTaskId(anomalousTask.UserId, anomalousTask.TaskID)
	require.NoError(t, err)
	require.True(t, exists)
	assert.Equal(t, anomalousReservation.ReservationID, reloadedAnomalous.PrivateData.EdgeReservationID)
	reloadedHealthy, exists, err := model.GetByTaskId(healthyTask.UserId, healthyTask.TaskID)
	require.NoError(t, err)
	require.True(t, exists)
