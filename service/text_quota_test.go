...
func TestCalculateTextQuotaSummaryUsesPinnedEdgeQuotaUnit(t *testing.T) {
	...
	relayInfo := &relaycommon.RelayInfo{
		...
		EdgePricingPolicy: &edgedto.EdgePricingPolicyV1{
			PolicyID: "edge-fixed", Version: "v1", Model: "gpt-edge-fixed",
			BillingMode: edgedto.EdgeBillingModeFixedPriceV1, ModelPrice: &price, QuotaPerUnit: 100,
		},
		StartTime: time.Now(),
	}
...
func TestCalculateTextToolSurchargeUsesPinnedEdgePolicy(t *testing.T) {
	require.NoError(t, common.SetRuntimeMode(common.RuntimeModeEdge))
	t.Cleanup(func() {
		require.NoError(t, common.SetRuntimeMode(common.RuntimeModeMaster))
	})
	...
	relayInfo := &relaycommon.RelayInfo{
		...
		EdgePricingPolicy: &edgedto.EdgePricingPolicyV1{
			ToolPrices:   map[string]float64{dto.BuildInToolGoogleSearch: 20_000},
			QuotaPerUnit: 100,
		},
		StartTime: time.Now(),
	}
	...
	summary := calculateTextQuotaSummary(ctx, relayInfo, &dto.Usage{})
	require.Equal(t, 2_000, summary.Quota)
	require.Equal(t, []ToolSurchargeItem{{Name: dto.BuildInToolGoogleSearch, Count: 1, Price: 20_000}}, summary.ToolSurchargeItems)
}
